...
{  
	...
	I2S3ext_InitStructure.I2S_Mode=I2S_Mode^(1<<8);//IIS模式
	I2S3ext_InitStructure.I2S_Standard=I2S_Standard;//IIS标准
	I2S3ext_InitStructure.I2S_DataFormat=I2S_DataFormat;//IIS数据长度
	I2S3ext_InitStructure.I2S_MCLKOutput=I2S_MCLKOutput_Disable;//主时钟输出禁止
	I2S3ext_InitStructure.I2S_AudioFreq=I2S_AudioFreq_Default;//IIS频率设置
	I2S3ext_InitStructure.I2S_CPOL=I2S_Clock_Polarity;//空闲状态时钟电平
	
	I2S_FullDuplexConfig(I2S3ext,&I2S3ext_InitStructure);//初始化I2S2ext配置
	
	SPI_I2S_DMACmd(I2S3ext,SPI_I2S_DMAReq_Rx,ENABLE);//I2S2ext RX DMA请求使能.
 
	I2S_Cmd(I2S3ext,ENABLE);		//I2S2ext I2S EN使能.
...
{  
  ...
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//外设非增量模式
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;//存储器增量模式
  DMA_InitStructure.DMA_PeripheralDataSize = DMA_MemoryDataSize_HalfWord;//外设数据长度:16位
  DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;//存储器数据长度：16位 
  DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;// 使用循环模式 
  DMA_InitStructure.DMA_Priority = DMA_Priority_High;//高优先级
  DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable; //不使用FIFO模式        
...
{ 	
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1,ENABLE);//DMA1时钟使能 
	...
	while (DMA_GetCmdStatus(DMA1_Stream0) != DISABLE){}//等待DMA1_Stream3可配置 
		
	DMA_ClearITPendingBit(DMA1_Stream0,DMA_IT_FEIF0|DMA_IT_DMEIF0|DMA_IT_TEIF0|DMA_IT_HTIF0|DMA_IT_TCIF0);//清空DMA1_Stream3上所有中断标志

  /* 配置 DMA Stream */

  DMA_InitStructure.DMA_Channel = DMA_Channel_3;  //通道3 I2S2ext_RX通道 
  DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&I2S3ext->DR;//外设地址为:(u32)&I2S2ext->DR>DR
  DMA_InitStructure.DMA_Memory0BaseAddr = (u32)buf0;//DMA 存储器0地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;//外设到存储器模式
  DMA_InitStructure.DMA_BufferSize = num;//数据传输量 
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//外设非增量模式
  ...
  DMA_ITConfig(DMA1_Stream0,DMA_IT_TC,ENABLE);//开启传输完成中断
...
void (*i2s3_tx_callback)(void);	//TX回调函数
void (*i2s3_rx_callback)(void);	//RX回调函数
...
{      
	...
	{ 
		DMA_ClearITPendingBit(DMA1_Stream5,DMA_IT_TCIF5);
      	i2s3_tx_callback();	//执行回调函数,读取数据等操作在这里面处理  
	}   											 
} 
//DMA1_Stream3中断服务函数
...
{      
	...
	{ 
		DMA_ClearITPendingBit(DMA1_Stream0,DMA_IT_TCIF0);	//清除传输完成中断
      	i2s3_rx_callback();	//执行回调函数,读取数据等操作在这里面处理  
	}  											 
}
//I2S开始播放
...
void I2S3_Rec_Start(void)
{   	    
	DMA_Cmd(DMA1_Stream0,ENABLE);//开启DMA TX传输,开始录音	
}
//关闭I2S录音
...
void I2S3_Rec_Stop(void)
{  
	DMA_Cmd(DMA1_Stream0,DISABLE);//关闭DMA,结束录音	  
}
